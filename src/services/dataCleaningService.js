...
/**
 * Clean data using Python data cleaning module
 * @param {Object} data - Data to clean
 * @param {Function} progressCallback - Callback for progress updates
 * @returns {Promise<Object>} - Cleaned data and cleaning log
 */
export const cleanData = async (data, progressCallback) => {
  try {
...
import pyodide.code
data_cleaning_code = '''
${getPythonCleaningModule()}
'''
namespace = {}
pyodide.code.eval_code(data_cleaning_code, namespace)
DataCleaner = namespace['DataCleaner']
clean_dataset = namespace['clean_dataset']
...
if 'datasets' in data and data['datasets']:
    ...
    for dataset in data['datasets']:
        try:
            # Clean the dataset
            dataset_type = dataset.get('type', 'csv')
            result = clean_dataset(dataset['data'], file_type=dataset_type)
            
            # Update the dataset with cleaned data
            dataset['data'] = result['cleaned_data']
            
            # Add cleaning summary to dataset
            dataset['quality'] = result['summary'].get('column_metadata', {})
            
            # Add cleaning log entries
            cleaning_log.extend(result['summary'].get('cleaning_log', []))
            
            cleaned_datasets.append(dataset)
        except Exception as e:
            console.error(f"Error cleaning dataset {dataset.get('name')}: {str(e)}")
            # Keep original dataset if cleaning fails
            cleaned_datasets.append(dataset)
    
    cleaned_data['datasets'] = cleaned_datasets
...
json.dumps({
    ...
    // Parse the result
    const cleaningResult = JSON.parse(result);
...
/**
 * Get Python data cleaning module code
 * This contains the same code as data_cleaning.py but we include it here to avoid needing to load the file separately
 * @returns {string} - Python code for data cleaning
 */
function getPythonCleaningModule() {
  return `
import pandas as pd
import numpy as np
